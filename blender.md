...
### >---------- NEW BLENDER ----------
Q1: When I umpack my textures, many of the invididual files are packed into larger combined files!

> A1: This can happen if you're using the standalone version of Blender. The steam version of Blender does not seem to have this issue.
...
Q3: When I'm trying to export with bodygroups, I get an error along the lines of "too many 96 flex controllers on x".

> A3: AFAIK, a model can have up to 96 flex controllers. You may have more than 96, or you may have combined and re-separated parts of the model such that separate parts both have flexes (ex. the body mesh has flexes.) This puts the flex controller count over the limit, so you must ensure flexes (shape keys in blender) are only present on the face mesh, where they're used.

### >---------- 2.79 BLENDER ----------
Q1: RuntimeError: Armature 'proportions' not in edit mode, cannot add an editbone

> A1: When you're importing the .smd, make sure to first set 'Bone Append Mode' to 'Make New Armature', then import the following model pieces with 'Append to Target'.

Q2: UnicodeDecodeError: 'utf-8' codec can't decode byte 0xd5 in position 0: unexpected end of data
